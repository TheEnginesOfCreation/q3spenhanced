...
static int GametypeBits( char *string ) {
	...
	while( 1 ) {
		...
		if (Q_stricmp(token, "spt") == 0) {
			bits |= 1 << GT_SINGLE_PLAYER_TEAM;
			continue;
		}

		if (Q_stricmp(token, "spc") == 0) {
			bits |= 1 << GT_SINGLE_PLAYER_CTF;
			continue;
		}
...
static qboolean BotAlreadySelected( const char *checkName ) {
	...
	for( n = 1; n < PLAYER_SLOTS; n++ ) {
		...
		if( (GT_IsTeam(s_serveroptions.gametype)) &&
...
static void ServerOptions_Start( void ) {
	...
	// set player's team
	if( dedicated == 0 && GT_IsTeam(s_serveroptions.gametype)) {
...
static void ServerOptions_InitPlayerItems( void ) {
	...
	if( s_serveroptions.multiplayer && !GT_IsTeam(s_serveroptions.gametype) ) {
